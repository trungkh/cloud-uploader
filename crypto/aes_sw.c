...
static uint32_t FSb[256] = { 0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5,
...
static uint32_t RSb[256] = { 0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5, 0x38,
...
int aes_set_key(struct aes_context *ctx, uint8_t *key, int nbits,
        const char *mode)
{
    int i;
    uint32_t *RK;
    ...
    case 128:
        ...
        for (i = 0; i < 10; i++, RK += 4)
        {
            RK[4] = RK[0] ^ RCON[i] ^ (FSb[(RK[3] >> 8) & 0xFF])
                    ^ (FSb[(RK[3] >> 16) & 0xFF] << 8)
                    ^ (FSb[(RK[3] >> 24)] << 16) ^ (FSb[(RK[3]) & 0xFF] << 24);
    ...
    case 192:
        ...
        for (i = 0; i < 8; i++, RK += 6)
        {
            RK[6] = RK[0] ^ RCON[i] ^ (FSb[(RK[5] >> 8) & 0xFF])
                    ^ (FSb[(RK[5] >> 16) & 0xFF] << 8)
                    ^ (FSb[(RK[5] >> 24)] << 16) ^ (FSb[(RK[5]) & 0xFF] << 24);
    ...
    case 256:
        ...
        for (i = 0; i < 7; i++, RK += 8)
        {
            RK[8] = RK[0] ^ RCON[i] ^ (FSb[(RK[7] >> 8) & 0xFF])
                    ^ (FSb[(RK[7] >> 16) & 0xFF] << 8)
                    ^ (FSb[(RK[7] >> 24)] << 16) ^ (FSb[(RK[7]) & 0xFF] << 24);
...
void aes_encrypt(struct aes_context *ctx, uint8_t iv[16], uint8_t* data,
        uint32_t length)
{
...
void aes_decrypt(struct aes_context *ctx, uint8_t iv[16], uint8_t* data,
        uint32_t length)
{
