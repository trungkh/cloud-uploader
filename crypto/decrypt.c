 ...
 * DESCRIPT: None
 */
static size_t alawDataDecrypt(uint8_t* key, uint8_t* iv, uint8_t* data,
        size_t length)
{
    size_t outLength = 0;
 ...
 */
size_t alawDecryptWrapper(void* frame, size_t frameLength)
{
    return alawDataDecrypt(gAlawDataEncryptKey, gAlawDataEncryptIV, frame,
            frameLength);
}
 ...
 * DESCRIPT: None
 */
int alawDecryptKeyInit(uint8_t *key, uint8_t *iv)
{
    /* Init key */
